...
stopuser = {}
token = '6508582139:AAETVv6u9mfGgUrxlnsY52zDSxq8WwpRfuo'
    ...
    def my_function():
      ...
      "plan": "Free - Not Subscribed",
        ...
        if BL == 'Free - Not Subscribed':    
            ...
            contact_button = types.InlineKeyboardButton(text="ϟ Programmer - ddfmoto0", url="https://t.me/ddfmoto0")
            keyboard.add(contact_button)
            random_number = random.randint(4, 17)
            photo_url = f'https://t.me/hitzzas/4'
...
ϟ Programmer ~ @ddfmoto0 </b>''',reply_markup=keyboard)
            return
        keyboard = types.InlineKeyboardMarkup()
...
ϟ - Programmer • @ddfmoto0 ϟ</strong>''',reply_markup=keyboard)
...
- Programmer ~ @ddfmoto0 - </b>''',reply_markup=keyboard)
            return
        with open('data.json', 'r') as file:
            json_data = json.load(file)
...
- Programmer ~ @ddfmoto0 -  </b>''',reply_markup=keyboard)
        ...
        ''',reply_markup=keyboard)
            ...
            return
        keyboard = types.InlineKeyboardMarkup()
        contact_button = types.InlineKeyboardButton(text=f"ϟ Stripe Donate 1$ ϟ",callback_data='br')
        contact_button2 = types.InlineKeyboardButton(text=f"ϟ Braintree Charge 5$ ϟ",callback_data='br2')
        keyboard.add(contact_button,contact_button2)
...
def show_menu(message):
    if str(message.from_user.id) in admins:
        ...
        br3_button = types.InlineKeyboardButton(f" 3 ({toggle_br3})", callback_data='toggle_br3')
...
def menu_callback(call):
    ...
    def my_function():
        ...
        cm3 = types.InlineKeyboardButton("- 𝐂𝐇𝐀𝐑𝐆𝐄 ✅-> Number ", callback_data='x')
        cm4 = types.InlineKeyboardButton("- 𝗗𝗘𝗖𝗜𝗡𝗘𝗗 ❌-> Number ", callback_data='x')
        ...
        try:
            with open("combo.txt", 'r') as file:
                ...
                for cc in lines:
                    ...
                    cm3 = types.InlineKeyboardButton(f"- 𝐂𝐇𝐀𝐑𝐆𝐄 !✅ • {live}", callback_data='x')
...
ϟ - Programmer -> @ddfmoto0⚡</b>'''

                    if "Funds" in last or 'Invalid postal' in last or 'charge' in last or 'added' in last or 'Duplicate' in last or 'Approved' in last or 'CVV' in last:
                        live += 1
...
def menu_callback(call):
    ...
    def my_function():
        ...
        cm1 = types.InlineKeyboardButton("- [ CARD ]", callback_data='u8')
        status = types.InlineKeyboardButton(f"- Status & Message ", callback_data='u8')
        cm3 = types.InlineKeyboardButton("- CHARGE ✅-> Number ", callback_data='x')
        ...
        try:
            with open("combo.txt", 'r') as file:
                ...
                for cc in lines:
                    ...
                    cm1 = types.InlineKeyboardButton(f"- 𝘾𝘾 • {cc}", callback_data='u8')
                    status = types.InlineKeyboardButton(f"- 𝙎𝙩𝙖𝙩𝙪𝙨 • {last}", callback_data='u8')
                    cm3 = types.InlineKeyboardButton(f"- CHARGE !✅ • {live}", callback_data='x')
...
ϟ - Programmer -> @ddfmoto0⚡</b>'''
                    ...
                    if  'Invalid postal' in last or 'charge' in last or 'added' in last or 'Duplicate' in last or 'Approved'  in last:
                        live += 1
                        bot.send_message(call.from_user.id, msg)
                    else:
                        dd += 1
...
┣🤖 Bot by: <a href="https://telegram.dog/ddfmoto0">ddfmoto0</a></b>'''
                    cvc = f'''<b>• Ccn Card ☑️        
┏━✨ CARD DETAILS ✨━
┣💳 Card:<code>{cc}</code>
┣📋 Status: {last} 🟢
...
┣🤖 Bot by: <a href="https://telegram.dog/ddfmoto0">ddfmoto0</a></b>'''
                    ...
                    if "Funds" in last or 'Invalid postal' in last or 'charge' in last or 'added' in last or 'Duplicate' in last or 'Approved' in last or 'cheeta' in last:
                        live += 1
                        bot.send_message(call.from_user.id, msg)
                    elif 'CVV' in last:
                        live += 1
...
You are Not Subscribed ddfmoto0Chk BOT !❌
...
- Programmer • @ddfmoto0</b>'''
        ...
        bot.reply_to(message, f'''- Welcome Dear ♡!
You are Not Subscribed ddfmoto0 BOT !❌
...
┣🤖 Bot by: <a href="https://telegram.dog/ddfmoto0">ddfmoto0</a></b>'''
        ...
        bot.reply_to(message, f'''- Welcome Dear ♡!
You are Not Subscribed ddfmoto0 Chk BOT !❌
...
def qwwe(message):
    if str(message.from_user.id) in myid:
        ...
        try:
            ...
            bot.reply_to(message, f'''- Hello ddfmoto0
• Done Send - {mp}
• Bad Send - {erop}''')
...
Note Only One gate is working  charge 16$
...
def generate_code(message):
    def my_function():
        ...
        try:
            ...
            pas = 'DD-' + '2025' +'-' + ''.join(random.choices(characters, k=4))
            current_time = datetime.now()
            ...
            msg = f'''<b>ϟ ddfmoto0 checker Code
